...
public class Set extends JFrame implements ActionListener{
	...
	public Set()
	{
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					ResultSet rs=stm.executeQuery(sql);
							...
							while (rs.next()) {
								sql="update user_table set password='123' where ID='25'";
								int i=stm.executeUpdate(sql);
				    	         if(i>0)JOptionPane.showMessageDialog(null,"修改成功！");
		...
		JBSet2.setBounds(530,570,90,20);
		this.add(JBSet2);
		JBSet2.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					ResultSet rs=stm.executeQuery(sql);
							...
							while (rs.next()) {
								sql="update train_table set startTime='13:00' where tid='D263'";	
								int i=stm.executeUpdate(sql);
				    	         if(i>0)JOptionPane.showMessageDialog(null,"修改成功！");
				    	         else JOptionPane.showMessageDialog(null,"修改失败！");
							}
		...
		JBExit.setBounds(350,570,90,20);
		this.add(JBExit);
